...
if uploaded_file is not None:
    ...
    if st.button("Deteksi Audio"):
        ...
        with open('scaler.pkl', 'rb') as scal:
            loadscal = pickle.load(scal)
        
        loadscal.fit(X_train[kolom])
        X_train_scaled = loadscal.transform(X_train[kolom])
        # Lakukan standarisasi pada kolom yang telah ditentukan
        data_ternormalisasi = loadscal.transform(data_mentah[kolom])
        

       # with open('MinMaxScaler.pkl', 'rb') as mix:
           # loadmix = pickle.load(mix)

       # loadmix.fit(X_train[kolom])
       # data_MinimMaxim = loadmix.transform(data_mentah[kolom])
        # Reduksi dimensi menggunakan PCA
 # Load the PCA model from the pickle file
        with open('PCA1.pkl', 'rb') as pc:
            loadpc = pickle.load(pc)

        sklearn_pca = loadpc
        X_train_pca = sklearn_pca.fit_transform(X_train_scaled)
        X_pca = sklearn_pca.transform(data_ternormalisasi)
       #6 MM_pca = sklearn_pca.transform(data_MinimMaxim)
